...
if(isset($_POST['submit'])){
    ...
    $select_users = mysqli_query($conn, "SELECT * FROM `client` WHERE email = '$client_email' AND nid = '$client_nid'") or die('query failed');
    if(mysqli_num_rows($select_users) > 0){
        $message[] = 'Your application is on process';
    }else{

        
        $query="INSERT INTO `client`(name, email, phone, address, birthdate, nid, sex, marital_status, policy_no, agent_id) VALUES('$client_name', '$client_email', '$client_phone', '$address','$client_birthdate','$client_nid','$client_sex','$marital_status','$policy_number','$admin_id')";
        
        mysqli_query($conn,$query);

        $query="INSERT INTO `nominee`(nominee_nid, client_nid, name, sex, birth_date, relation, phone) VALUES('$nominee_nid', '$client_nid, '$nominee_name','$nominee_sex','$nominee_birthdate','$relation', '$nominee_phone')" or die('query trashed');
